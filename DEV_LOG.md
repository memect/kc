...
Design doc: [`docs/update_design_v7.md`](./docs/update_design_v7.md).

**The headline finding from E2E #5** (DS + GLM audits in
`archive/e2e_test_20260428_{ds,glm}_session_audit.md`): the
hard-tracking layer never constrained either alive contestant —
12/12 phase transitions force-bypassed, engine milestones at zero
across phases that had real work on disk, both bundles broken.
Tracking-as-ground-truth from v0.6.1 failed silently because tracking
was wired to milestone-recording tool wrappers, and agents who use
Write/Bash/sandbox_exec directly bypass them.

**v0.7.0 fixes this by inverting the relationship.** Engine derives
milestones from filesystem facts (disk wins over tool-call
instrumentation); agent owns TaskBoard decomposition (engine no
longer auto-creates per-rule tasks); methodology lives in a new
`work-decomposition` meta-meta skill the agent reads at the right
phase. The thin-harness/fat-skills principle and the hard-tracking/
soft-executing principle become real instead of aspirational.

### Group K — license migration (PolyForm Noncommercial 1.0.0)

KC switches from MIT to **PolyForm Noncommercial 1.0.0** + a separate
commercial license. Three constraints driving the change: (a) personal
users free, (b) no republish-as-new-product, (c) enterprises in
production pay for a commercial license.

- New [`LICENSE`](./LICENSE) — PolyForm Noncommercial 1.0.0 verbatim,
  with a `Required Notice:` block appended pointing at LICENSE-COMMERCIAL.md.
- New [`LICENSE-COMMERCIAL.md`](./LICENSE-COMMERCIAL.md) — what counts
  as commercial use, contact heavysal@gmail.com for commercial licensing.
- `package.json` — `"license": "MIT"` → `"license": "SEE LICENSE IN LICENSE"`.
- Welcome banner gets a one-line license footer.

Historical commits stay under MIT for those release versions; v0.7.0+
is PolyForm. KC has no external contributors, so relicensing future
commits is uncontroversial.

`/meme` easter egg defensive obfuscation (K2): canonical source moved
to `src/cli/meme.source.js` (excluded from npm via `package.json:files`).
Generated `src/cli/meme.js` ships LYRICS / TEAM / watermark as a
base64 + XOR blob with inline decoder. `npm run build:meme` regenerates
from source; `prepublishOnly` runs it automatically. Bytenode (original
plan) replaced by encoded-source — meme.js imports react+ink (ESM only)
and bytenode targets CJS, so bridging would need an esbuild bundle dep.
Encoded-source achieves the same grep-resistance with zero extra deps.
112-char watermark survives in the decoded payload to identify origin
in copies. `grep -c kitchen-engineer42 src/cli/meme.js` → 0;
`src/cli/meme.source.js` → 3.

### Group A — hard-tracking floor (the architectural payload)

New `src/agent/pipelines/_milestone-derive.js` centralizes filesystem
checks for every phase. All seven pipelines route through it. Tool-
wrapper recorders stay as auxiliary signal but no longer authoritative.

Companion fixes:
- `forced` flag in `phase_transition` events fixed: `force && nextPhase
  !== expected` → `!!force`. E2E #5 audit logs claimed 0/12 forced
  when reality was 12/12.
- Refusal text scrubbed of `force:true` advertising — old refusal
  trained the agent to bypass; new refusal points at /status +
  describeState + missing-milestone disk artifacts.
- Skill validator runs on every `workspace_file write` to rule_skills
  paths (was: only at exitCriteriaMet, which got force-bypassed).
  Failures surface in describeState within one tool-call cycle.
- TaskManager.reconcileAgainstDisk: phase boundaries flip "completed"
  tasks back to pending if helper-derived ruleIdsCovered doesn't
  include them. Catches the E2E #5 DS gap (70/70 done but only 56
  dirs on disk).
- Validator regex loosened to any top-level `def` — the v0.6.2 strict
  `check_rule|verify` rejected 27/28 GLM scripts (false negatives
  on style alone).
- `rulesWithTests` accepts multiple test-artifact shapes (test_cases/,
  tests/, check*.py, scripts/check*.py, assets/test_cases.json) — the
  v0.6.x form-prescriptive criterion blocked all three E2E #5
  contestants who each picked a different convention.

**Offline replay against E2E #5 saved workspaces**: force-bypass would
drop from 12/12 to ~8/12, with the remaining forces having honest
reasons (DS genuinely didn't write coverage_audit.md or do chunk refs;
GLM genuinely skipped real skill testing in 5 minutes) rather than
gate misalignment.

### Group H — static-audit critical + high (defensive)

Three critical + nine high from the 2026-04-28 audit. C2 (skill-testing
exit gate accuracy bug), H4 (scripts/ dir bug), H5 (production-qc
vacuous-truth flip) collapsed into Group A's filesystem-derived
rewrite. Remaining shipped:

- **C3** StatusBar ref mutation moved to useEffect (StrictMode-safe)
- **H1** trackedPromise tail .catch (defensive against secondary throws)
- **H3** withSyncFileLock uses resolvePath (path-traversal safe)
- **H6** showWelcome dismisses on first non-system message
- **H7** /compact IIFE has top-level .catch
- **H8** /resume IIFE same
- **H9** loadEnvFile try/catch (no longer crashes bootstrap on
  permission-denied .env)

C1, H1, H2 — audit was overstated; defensive comments added explaining
why existing code is safe enough.

### Group B — agent-owned decomposition + work-decomposition skill

New `template/skills/{zh,en}/meta-meta/work-decomposition/SKILL.md`
(~400 lines per language). Teaches:
- Ordering methodologies — Shannon-Huffman (recommended default), BFS,
  DFS, binary partition. With when-to-pick + traps for each.
- Grouping rules — bundle-when (shared chunk + shared input + shared
  judgment + co-failure) vs keep-separate-when (different chapters /
  tiers / doc types / parent-child relations).
- Three-axis difficulty triage (CoT depth + module count + cross-rule
  interaction). Output: `rules/difficulty.json`.
- PATTERNS.md memory discipline — what to write (transferable shape,
  project constraint, anti-pattern w/ rationale) vs NOT (completion
  log, tool-history echo, FS-authoritative facts, conversation summary).
  5 KB cap.

`TaskManager.PER_RULE_PHASES` is now empty by default. Engine no
longer auto-creates per-rule tasks. Agent owns decomposition shape.
Feature flag `KC_AGENT_OWNS_TASKBOARD=0` restores v0.6.x behavior for
staged rollout (slated for removal in v0.8.0).

PATTERNS.md plumbing:
- `engine._readProjectMemory()` reads `rules/PATTERNS.md` (5 KB cap
  with truncation marker).
- `ContextAssembler.build()` takes `projectMemory` param; renders as
  a "Project memory" block in the system prompt for skill_authoring +
  skill_testing only.

**Tradeoff to flag**: more freedom widens the strong/weak conductor
gap. Mitigation is Group A's filesystem-derived floor — bad grouping
fails the gate; user sees missing milestones honestly in /status.

### Group D — skill format alignment

D1 — skill_authoring describeState now inlines the canonical skill
folder layout spec directly into every system prompt of that phase.
~250 token cost; benefit is first-attempt structural compliance
(E2E #5: 3 of 4 contestants ignored the spec because reading it
required navigation).

D2 — skill-creator description rewritten in zh + en frontmatter. Now
explicit about its role: Anthropic's scaffolding toolkit (apply after
KC's per-rule skills exist), not the primary reference for building
them. Read `meta-meta/skill-authoring` + `meta-meta/work-decomposition`
first. Directory rename deferred (would touch 6 src files + npm
package convention; clearer description achieves the scope clarity).

### Group F — filesystem & UX polish

- F1 — `Workspace.fsCaseSensitive` detected at construction. Foundation
  for case-collision warnings; the actual warning hookup in
  workspace_file deferred to v0.7.1.
- F2 — /resume now populates TUI TaskBoard from the resumed engine's
  taskManager (was: empty after resume even when tasks.json had work).
- F3 — `Scheduler.pendingInputCount()` excludes hidden files. Agent
  scratch goes under `input/.kc-scratch/` (sidecar marker).

### Group E partial — per-provider contextLimit (#96)

`providerContextCap` field on providers.js entries. Effective
contextLimit clamps `min(requested, cap)` with a console warning.
SiliconFlow gets cap=200000 — prevents future GLM-5.1 sessions from
hitting the deployment hard ceiling that broke E2E #5 GLM at 203,363
tokens with KC_CONTEXT_LIMIT=400000.

### Group C/E partial — release tool layout fallback (#98) + budget-aware compact (#93)

- `release._findLatestWorkflow` falls through canonical subdir,
  DS/GLM flat layout (`workflows/<id>_workflow.py`), and DS regex_skill
  manifest (`workflows/<id>.json` with `.entry` pointer). "no workflows
  found" error rewritten to spell out accepted layouts + actionable
  next steps. E2E #5 GLM dismissed the previous error with "不影响
  系统功能" and shipped a broken bundle; the new error makes that
  harder.
- `_runTaskLoopSerial` compact threshold: `messages.length > 15` →
...
- package.json: `0.6.2` → `0.7.0` (was staged at `0.7.0-pre` since
  Group K).
- This DEV_LOG entry.
