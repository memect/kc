...
## v0.7.0 (in progress, 2026-04-29 →)

Architectural overhaul after E2E #5. Plan:
[`docs/update_design_v7.md`](./docs/update_design_v7.md) (drafted in
v0.7.0 Group J). Eleven groups; **K lands first** so all subsequent
commits ship under the new license.

### License migration — Group K1 (2026-04-29)
...
- New [`LICENSE`](./LICENSE) — PolyForm Noncommercial 1.0.0 verbatim,
  with a `Required Notice:` block appended pointing at
  LICENSE-COMMERCIAL.md.
- New [`LICENSE-COMMERCIAL.md`](./LICENSE-COMMERCIAL.md) — what counts
  as commercial use, what counts as noncommercial, how to request a
  commercial license (heavysal@gmail.com), licensee compliance,
  violation reporting.
- `package.json` — `"license": "MIT"` → `"license": "SEE LICENSE IN LICENSE"`
  (PolyForm not in SPDX index, so this is the npm-correct convention).
  Description and version bumped to flag the dual-license model and
  the v0.7.0-pre staging tag.
- README License section rewritten with the three-track summary.
- Welcome banner gets a single-line license footer so first-run users
  see the model immediately.

**Historical commits remain MIT.** Per OSS practice, license changes
aren't retroactive: every recipient of v0.6.x npm packages keeps MIT
rights to those versions. Only post-cutover commits and v0.7.0+
releases are PolyForm. KC has no external contributors, so future
commits are uncontroversially relicensable.

### `/meme` defensive obfuscation — Group K2 (2026-04-29)

The `/meme` easter egg ships the team handles + author watermark.
v0.7.0 splits it into two files so the npm package no longer ships a
plaintext list of contributor handles or the watermark string —
defeating casual `grep`-based plagiarism scraping.

- `src/cli/meme.source.js` — canonical dev source. Plain text, kept
  in repo for humans to edit. **Excluded** from the npm package via
  `package.json:files`.
- `src/cli/meme.js` — generated by `scripts/build-meme.js`. Same
  React/Ink component shell, but LYRICS / TEAM / `__KC_MEME_WATERMARK__`
  are encoded as a base64 + XOR blob with an inline decoder.
  Committed to repo so dev mode works without running build first;
  shipped to npm.
- `npm run build:meme` regenerates `meme.js` from `meme.source.js`.
  `prepublishOnly` runs it automatically before publish.

**Why not bytenode** (the original plan picked bytenode): meme.js
imports react + ink, both ESM-only. bytenode targets CJS bytecode,
and bridging ESM imports through V8 bytecode would require an
esbuild bundle step + a heavy build dep. Encoded-source achieves the
same defensive goal (the team handles aren't grep-able in the
shipped form) with no extra deps and no Node-version brittleness.

**Watermark stays.** A 112-char watermark string identifies origin
even after disassembly. Tampering with it leaves a visible gap in
the `/meme` output. Please don't strip it from forks.

`grep -c kitchen-engineer42 src/cli/meme.js` returns 0 in the shipped
form; `grep -c kitchen-engineer42 src/cli/meme.source.js` returns 3
in the dev source.
